...
*Задача*: Модель должна предсказать температуры стали (задача регрессии).     
*Данные и фичи*: данные с параметрами составляющих для изготовления стали (первая измеренная температура, полная мощность и т.п.). Целевой признак: последняя измеренная температура.    
*Стек инструментов*: Отдельные выборки с составляющими стали подлежащие обработки и по итогу проведение их merge в общую таблицу признаков, построение корреляционной матрицы для исключения мультиколлинеарности признаков, проведение train_test_split, применение cross_val_score. Вычислена метрика MAE для проверки лучшей модели на train и test.    
*Результаты*: Проект со сложным, циклическим, технологическим процессом и большим количеством составляющих добавок. Метрикой MAE определена лучшая модель CatBoostRegressor (MAE достигнута установленного норматива по заданию).  
[*Проект*](https://github.com/k-vero2021/Training_project_2021/blob/main/%D0%9F%D1%80%D0%B5%D0%B4%D1%81%D0%BA%D0%B0%D0%B7%D0%B0%D0%BD%D0%B8%D0%B5%20%D1%82%D0%B5%D0%BC%D0%BF%D0%B5%D1%80%D0%B0%D1%82%D1%83%D1%80%D1%8B%20%D1%81%D1%82%D0%B0%D0%BB%D0%B8.ipynb)

***Прогнозирование заказов такси.***  
*Задача*: Построить модель для прогноза количества заказов такси на следующий час (задача регрессии, временные ряды).  
